###############################
##### Create etctnasyncdownload-polljobLambda in UAT Environment #####
###############################
...
resource "aws_lambda_function" "etctnasyncdownload-polljob_lambda_function" {
  ...
  package_type  = "Image"
  image_uri     = "${var.aws_accounts.theradex-shared-service.account_id}.dkr.ecr.us-east-1.amazonaws.com/uat-etctnasyncdownload-polljob-lambda:latest"
  environment {
    variables = {
