...
locals {
  private_subnet_ids = [
    for az, id in module.prod_comm_vpc.private_subnet_ids: 
      id   
  ]
  public_subnets_ids = [
    for az, id in module.prod_comm_vpc.public_subnet_ids: 
      id   
  ]
  myaccount = var.aws_accounts.theradex-production-commercial.account_id
}
...
variable "qrcg_cluster_name" {
    ...
    type        = string
    default     = "theradex-production-comm-cluster"
}
