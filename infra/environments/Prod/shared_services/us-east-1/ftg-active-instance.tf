...
resource "aws_network_interface" "eth0" {
  ...
  subnet_id   = module.security_vpc.public_subnet_ids[element(keys(module.security_vpc.public_subnet_ids), 0)]
  private_ips = [var.activeport1]
}

resource "aws_network_interface" "eth1" {
  description       = "active-port2"
  subnet_id         = module.security_vpc.private_subnet_ids[element(keys(module.security_vpc.private_subnet_ids), 0)]
...
resource "aws_network_interface" "eth2" {
  ...
  subnet_id         = aws_subnet.security-vpc-us-east-1a-ftg-ha.id
...
resource "aws_network_interface" "eth3" {
  ...
  subnet_id   = aws_subnet.security-vpc-us-east-1a-ftg-mgmt.id
...
resource "aws_instance" "fgtactive" {
  ...
  iam_instance_profile = var.ftg-iam-role
