###############################
##### Create FileIngest-SaveMedidata in PROD Environment #####
###############################
...
resource "aws_lambda_function" "fileingest-savemedidata_lambda_function" {
  ...
  package_type  = "Image"
  image_uri     = "${var.aws_accounts.theradex-shared-service.account_id}.dkr.ecr.us-east-1.amazonaws.com/prod-fileingesttomedidata-savetomedidata-lambda:latest"
  environment {
    variables = {
