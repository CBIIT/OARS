...
resource "aws_network_interface" "passiveeth0" {
  ...
  subnet_id   = module.security_vpc.public_subnet_ids[element(keys(module.security_vpc.public_subnet_ids), 1)]
  private_ips = [var.passiveport1]
}

resource "aws_network_interface" "passiveeth1" {
  description       = "passive-port2"
  subnet_id         = module.security_vpc.private_subnet_ids[element(keys(module.security_vpc.private_subnet_ids), 1)]
...
resource "aws_network_interface" "passiveeth2" {
  ...
  subnet_id         = aws_subnet.security-vpc-us-east-1b-ftg-ha.id
...
resource "aws_network_interface" "passiveeth3" {
  ...
  subnet_id   = aws_subnet.security-vpc-us-east-1b-ftg-mgmt.id
