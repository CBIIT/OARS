###############################
##### Create FileIngest-GenerateMedidataODM in PROD Environment #####
###############################
...
resource "aws_lambda_function" "fileingest-generatemedidataodm_lambda_function" {
  ...
  package_type  = "Image"
  image_uri     = "${var.aws_accounts.theradex-shared-service.account_id}.dkr.ecr.us-east-1.amazonaws.com/prod-fileingesttomedidata-generatemedidataodm-lambda:latest"
  environment {
    variables = {
