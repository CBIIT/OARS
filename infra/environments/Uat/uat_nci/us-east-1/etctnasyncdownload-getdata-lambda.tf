###############################
##### Create etctnasyncdownload-getdataLambda in UAT Environment #####
###############################
...
resource "aws_lambda_function" "etctnasyncdownload-getdata_lambda_function" {
  ...
  package_type  = "Image"
  image_uri     = "${var.aws_accounts.theradex-shared-service.account_id}.dkr.ecr.us-east-1.amazonaws.com/uat-etctnasyncdownload-getdata-lambda:latest"
  environment {
    variables = {
