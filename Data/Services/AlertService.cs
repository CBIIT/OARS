...
namespace TheradexPortal.Data.Services
{
    public class AlertService : BaseService, IAlertService
    {
        ...
        {
            return await context.Alerts.Where(a => a.IsActive == active && a.AlertType == alertType).ToListAsync();
        }

        public async Task<IList<WRAlert>> GetAllActiveNotesAsync()
        {
            return await context.Alerts.Where(a => a.IsActive == active && a.AlertType == noteType).ToListAsync();
        }

        public async Task<IList<WRAlert>> GetActiveLoginAlertsAsync()
        {
            return await context.Alerts.Where(a => a.IsActive == active && a.AlertType == alertType && a.PageName == loginPage).OrderByDescending(a => a.WRAlertId).ToListAsync();
        }

        public async Task<IList<WRAlert>> GetActiveLoginNotesAsync()
        {
            return await context.Alerts.Where(a => a.IsActive == active && a.AlertType == noteType && a.PageName == loginPage).OrderByDescending(a => a.WRAlertId).ToListAsync();
        }

        public async Task<IList<WRAlert>> GetActiveDashboardAlertsByIdAsync(int dashboardId)
        {
            return await context.Alerts.Where(a => a.IsActive == active && a.AlertType == alertType && a.DashboardId == dashboardId).OrderByDescending(a => a.WRAlertId).ToListAsync();
        }

        public async Task<IList<WRAlert>> GetActiveDashboardNotesByIdAsync(int dashboardId)
        {
            return await context.Alerts.Where(a => a.IsActive == active && a.AlertType == noteType && a.DashboardId == dashboardId).OrderByDescending(a => a.WRAlertId).ToListAsync();
        }
    }
}
